...
class Person extends jsw.TypedJsObject {
  static final $decode = new jsw.TypedJsObjectDecoder<Person>((js.JsObject jsObject) => new Person.fromJsObject(jsObject));
  static final $codec = new jsw.TypedJsObjectCodec<Person>($decode);
  ...
  List<Person> get children => jsw.TypedJsArray.$getDecode(Person.$codec).convert($unsafe['children']);
  set father(Person father) => $unsafe['father'] = father.$unsafe;
  Person get father => Person.$decode($unsafe['father']);

  String sayHello() => $unsafe.callMethod('sayHello');
}

class Color extends jsw.IsEnum<String> {
  static final $codec = new jsw.SerializableCodec<Color, String>(_FINDER.find);
