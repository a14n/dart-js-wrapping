...
class TypedJsMap<V> extends TypedJsObject implements Map<String,V> {
  static final $decode = new TypedJsObjectDecoder<TypedJsMap>(
      (JsObject jsObject) => new TypedJsMap.fromJsObject(jsObject));

  static TypedJsObjectDecoder<TypedJsMap> $getDecode(Codec valueCodec) =>
      new TypedJsObjectDecoder<TypedJsMap>((JsObject jsObject) =>
          new TypedJsMap.fromJsObject(jsObject, valueCodec));
  ...
  @override Iterable<String> get keys =>
      TypedJsArray.$decode(context['Object'].callMethod('keys', [$unsafe]));
